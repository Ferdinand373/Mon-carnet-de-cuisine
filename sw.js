const CACHE_NAME = 'mon-carnet-v2-3-6-1-correction';
const APP_SHELL = ['./', './index.html', './mon-carnet-v17.png', './stabilisation-v236.js'];
const PATCH_SCRIPT = '<script src="./stabilisation-v236.js?v=2361" defer></script>';
...
async function injectStabilisation(response) {
  ...
  if (!html.includes('stabilisation-v236.js')) {
    // IMPORTANT : viser la DERNIÈRE balise </body>.
    // Le fichier index.html contient aussi le texte "</body>" dans le code d'impression.
    const closingBody = html.toLowerCase().lastIndexOf('</body>');
    html = closingBody >= 0
      ? html.slice(0, closingBody) + PATCH_SCRIPT + html.slice(closingBody)
      : html + PATCH_SCRIPT;
  }
